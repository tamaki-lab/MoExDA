...
from .hat_labeled_video_dataset import hat_labeled_video_data
from .my_labeled_video_dataset import my_labeled_video_data
...
def collate_for_hat_video(batch: Any) -> Tuple[Any, Any]:
    batch_dict = torch.utils.data.default_collate(batch)
    ret = {
        ActionSwapType.Original.value: batch_dict["video"],
        "label": batch_dict["label"],
        ActionSwapType.PersonInpainting.value: batch_dict["bg_video"],
        ActionSwapType.PersonOnly.value: batch_dict["human_seg_video"],
        "bg_label": batch_dict["bg_label"],
    }
    return ret
...
def video_folder(
    ...
    train_dataset = my_labeled_video_data(
    ...
    val_dataset = hat_labeled_video_data(
    ...
    train_dataset.classes = sorted([d.name for d in Path(root_train_dir).iterdir() if not d.name.startswith('.')])
    val_dataset.classes = sorted([d.name for d in Path(
        root_val_dir + '/' + "original_videos").iterdir() if not d.name.startswith('.')])
    ...
    val_loader = DataLoader(
        ...
        drop_last=False,
        num_workers=video_folder_info.num_workers,
        collate_fn=collate_for_hat_video,
    )
