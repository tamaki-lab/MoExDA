...
def configure_model(
    ...
    if model_info.model_name == 'resnet18':
        ...
        model = CLIP(model_info, class_to_ids)  # type: ignore[assignment]

    elif model_info.model_name == 'swin_transformer':
        model = SwinTransformer(model_info)  # type: ignore[assignment]

    elif model_info.model_name == 'DeiT':
        model = DeiT(model_info)  # type: ignore[assignment]

    elif model_info.model_name == 'CvT':
        model = Cvt(model_info)  # type: ignore[assignment]

    elif model_info.model_name == 'vivit':
        model = ViViT(model_info)  # type: ignore[assignment]

    elif model_info.model_name == 'video_vit':
        model = ViTbv(model_info)  # type: ignore[assignment]

    elif model_info.model_name == 'timeSformer':
        model = TimeSformer(model_info)  # type: ignore[assignment]

    elif model_info.model_name == 'Moexlayervit':
        model = MoExLayerViT(model_info)  # type: ignore[assignment]
